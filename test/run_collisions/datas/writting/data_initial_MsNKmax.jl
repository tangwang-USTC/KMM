...
isp33 = 1
x0_nuTi = zeros(3NKmax)
nuTi_vec!(x0_nuTi, NKmax, nai0[isp33], uai0[isp33], vthi0[isp33])
datas_nModa = zeros(1, 3NKmax + 3)
datas_nModa[1:3] = [t0, LM[isp33], NKmax]
...
x0_nuTi = zeros(3NKmax)
nuTi_vec!(x0_nuTi,NKmax, nai0[isp33], uai0[isp33], vthi0[isp33])
datas_nModb = zeros(1, 3NKmax + 3)
datas_nModb[1:3] = [t0, LM[isp33], NKmax]
...
if ns ≥ 3
    ...
    x0_nuTi = zeros(3NKmax)
    nuTi_vec!(x0_nuTi,NKmax, nai0[isp33], uai0[isp33], vthi0[isp33])
    datas_nModc = zeros(1, 3NKmax + 3)
    datas_nModc[1:3] = [t0, LM[isp33], NKmax]
    ...
    if ns ≥ 4
        ...
        x0_nuTi = zeros(3NKmax)
        nuTi_vec!(x0_nuTi,NKmax, nai0[isp33], uai0[isp33], vthi0[isp33])
        datas_nModd = zeros(1, 3NKmax + 3)
        datas_nModd[1:3] = [t0, LM[isp33], NKmax]
