...
include(joinpath(pathroot,"src\\Collision\\weightfunctions_vL.jl"))

μu = 1
"""
  When `u ≫ 0`, i.e., `u ≥ 0.3`,  up to order `ℓ = 6` and may be the worst fit model to the moments.

  When `u → 0`, i.e., `u ≤ 1e-3`, similar to `_L`, perfect for `ℓ ≥ 1` and strict for `ℓ = 0`.
"""

# ua > 0
LL1 = 2   # When `ℓ ≫ 1` up to `ymax ≤ 1e-10`;
...
if is_p0_immutabel == 0
    ...
    if 2 == 2
        ...
        elseif LL1 == 2
            ys0 = copy(fvLa0[:,LL1][xvec]) ./ vs0
        else
            ys0 = copy(fvLa0[:,LL1][xvec]) ./ vs0 .^ℓ
        end
