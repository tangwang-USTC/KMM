...
include(joinpath(pathroot,"src\\Collision\\weightfunctions_uL.jl"))
...
"""
   When `û ≫ 0.0`, i.e., `û > 0.1`, `LL1` could be used up to order `ℓ = 20`,
    which could be the best fit model to the moments when `LM1 ≤ 20`;
    When `LM1 > 20`, `_L` will be a better fit model
    which can up to `ℓ > 20` and limited by `yscale0 ~ 1e-10`.

   When `û → 0.0`, i.e.,  `û ≤ 1e-3`, it is will be worse for `∀ ℓ`,
   i.e., `p_noise_ratio ≤ 1e-1` which is not as good as version `FP0D1VCSSpline_weighted`.
   More strict constraint, i.e., `p_noise_ratio ≤ 1e-5` is required for `∀ ℓ`.
   The better fit model is in `_L_restart.jl`



   Else, when `0.0 ≪ û ≪ 1.0`,
"""

# ua > 0
LL1 =  1   # When `ℓ ≫ 1` up to `ℓ = 20`,
...
y_isp = iFv3
# y_isp = isp3
p_noise_ratio = 1e-9 # Relative error of the initial guess parameters `p`
isokk = 0        # [0,1,2,3,4]
ratio_eps = 1e-1
...
if is_p0_immutabel == 0
    ...
    if 2 == 2
        ...
        elseif LL1 == 2
            ys0 = copy(fvLa0[:,LL1][xvec]) ./ u
        else
            ys0 = copy(fvLa0[:,LL1][xvec]) ./ u .^ℓ
        end
    ...
    elseif isokk == 0
        ...
        if isokk == 2
            ...
            if isokk == 3
                ...
                counts,poly,modelM,~,MnDMk = fvLmodel(copy(vs0),copy(ys0),ℓ,u,uMax,atolrn;
