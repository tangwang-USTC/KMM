...
function king!(out, x; Ml0=Ml0,l=0,nModel=nModel)
    ...
    # out[nj] = sum((nh .* vhth.^j) .* (1 .+ ck[1] * ((uh ./ vhth).^2) .+ ck[2] * ((uh ./ vhth).^4) .+ 
    #                                        ck[3] * ((uh ./ vhth).^j))) - Ml0[nj]
    j = 8
    nj = j / 2 + 1 |> Int
    N = (j - l) / 2 |> Int
    k = 1:Int(j/2)
    ...
    out[nj] = sum((nh .* vhth.^j) .* (1 .+ [sum(ck .* (uh[s] ./ vhth[s]).^(2k)) for s in 1:nModel])) - Ml0[nj]
    # out[nj] = sum((nh .* vhth.^j) .* (1 .+ ck[1] * ((uh ./ vhth).^2) .+ ck[2] * ((uh ./ vhth).^4) .+ 
    #             ck[3] * ((uh ./ vhth).^6) .+ ck[4] * ((uh ./ vhth).^j))) - Ml0[nj]
    j = 10
    nj = j / 2 + 1 |> Int
    N = (j - l) / 2 |> Int
    k = 1:Int(j/2)
    ...
    out[nj] = sum((nh .* vhth.^j) .* (1 .+ [sum(ck .* (uh[s] ./ vhth[s]).^(2k)) for s in 1:nModel])) - Ml0[nj]
    # out[nj] = sum((nh .* vhth.^j) .* (1 .+ ck[1] * ((uh ./ vhth).^2) .+ ck[2] * ((uh ./ vhth).^4) .+ 
    #             ck[3] * ((uh ./ vhth).^6) .+ ck[4] * ((uh ./ vhth).^8) .+ ck[5] * ((uh ./ vhth).^j))) - Ml0[nj]
end
...
function king_g!(J, x; Ml0=Ml0,l=0)
  ...
  nj = 2 |> Int
  s = 1
  J[nj,1] = vhth[s]^2 + 2/3 * uh[s]^2
  J[nj,2] = 4/3 * nh[s] * uh[s]
  J[nj,3] = 2nh[s] * vhth[s]
  s = 2
  J[nj,4] = vhth[s]^2 + 2/3 * uh[s]^2
  J[nj,5] = 4/3 * nh[s] * uh[s]
  J[nj,6] = 2nh[s] * vhth[s]
  ...
  s = 2
  J[nj,4] = vhth[s]^4 + 4/3 * uh[s]^2 * vhth[s]^2 + 4/15 * uh[s]^4
  J[nj,5] = 8/3 * nh[s] * uh[s] * (vhth[s] ^2 + 2/5 * uh[s]^2)
  J[nj,6] = 4nh[s] * vhth[s] * (vhth[s] ^2 + 2/3 * uh[s]^2)
  ...
  s = 2
  J[nj,4] = vhth[s]^j * (1 + sum(ck .* (uh[s] / vhth[s]).^(2k)))
  J[nj,5] = 2nh[s] * vhth[s]^(j-1) * sum(k .* ck .* (uh[s] / vhth[s]).^(2k .- 1))
  J[nj,6] = j * nh[s] * vhth[s]^(j-1) * (1 + sum((1 .- 2/j * k) .* ck .* (uh[s] / vhth[s]).^(2k)))
