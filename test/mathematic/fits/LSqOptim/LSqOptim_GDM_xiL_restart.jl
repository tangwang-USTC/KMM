...
# up to order `ℓ = 14`
include(joinpath(pathroot,"src\\Collision\\weightfunctions_xiL.jl"))
...
"""
# ua > 0
LL1 = 11   # When `ℓ ≫ 1` up to `ymax ≤ 1e-9`;
...
if is_p0_immutabel == 0
    ...
    if 2 == 2
        ...
        elseif LL1 == 2
            ys0 = copy(fvLa0[:,LL1][xvec]) ./ (2u*vs0)
        else
            ys0 = copy(fvLa0[:,LL1][xvec]) ./ (2u*vs0).^ℓ
        end
