...
include(joinpath(pathroot,"src\\Collision\\weightfunctions0.jl"))
...
p_noise_abs = 3e-1 # Absolute error of the initial guess parameters `p0`.
...
if issolvefDM == 1
    if istol == 1
        @time pM1,sigmaM1, rnM1, modelM1,modelM1v0,nDM = fvLmodelDM(xs,ys,ℓ,u,uMax;p=p0,
                show_trace=show_trace,p_tol=x_tol,g_tol=g_tol,maxIterLM=maxIterLM)
    else
        @time pM1,sigmaM1, rnM1, modelM1,modelM1v0,nDM = fvLmodelDM(xs,ys,ℓ,u,uMax;p=p0,show_trace=show_trace)
    end
else
    pM1 = p
end
