...
@show (ℓ, nvs), fmtf2.(u) ,fmtf2.(p_noise_rel)
...
if issolvefDM == 1
    if istol == 1
        @time pM1,sigmaM1, rnM1, modelM1,nDM = fvLmodelDM(vs,ys;p=p0,
                show_trace=show_trace,p_tol=x_tol,g_tol=g_tol,maxIterLM=maxIterLM)
    else
        @time pM1,sigmaM1, rnM1, modelM1,nDM = fvLmodelDM(vs,ys,ℓ,u,uMax;p=p0,show_trace=show_trace)
    end
else
    pM1 = p
end
