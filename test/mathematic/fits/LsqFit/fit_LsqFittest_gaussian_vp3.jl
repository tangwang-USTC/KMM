...
include(joinpath(pathroot,"src\\Collision\\weightfunctions_2pv.jl"))

# Gaussian models for Maxwellian distribution
modelMexp(v,p) = exp.(- p[1] * (v .- p[2]).^2)
...
dnxs = 5
p_noise_ratio = 1e-0 # Relative error of the initial guess parameters `p0`
p_noise_abs = 1e-0 # Absolute error of the initial guess parameters `p0`.
...
if LL1 == LL1
    ...
    if isnvlevel0 == 1
        ...
        xs0 = copy(vGk)
        xs0[1] == 0.0 ? (xvec = 2:nck) : (xvec = 1:nck)
        xs0 =xs0[xvec]
...
if issolvefDM == 1
    if istol == 1
        @time pM1,sigmaM1, rnM1, modelM1,nDM = fvLmodelDM(xs,ys;p=p0,
                show_trace=show_trace,p_tol=x_tol,g_tol=g_tol,maxIterLM=maxIterLM)
    else
        @time pM1,sigmaM1, rnM1, modelM1,nDM = fvLmodelDM(xs,ys,ℓ,u,uMax;p=p0,show_trace=show_trace)
    end
else
    pM1 = p
end
