"""
 ...
 gives:
  ...
  outputs:
    LM, fvL = fvLDMz(vGk,LM,ns,nMod,nai,uai,vthi;
...
function fvLDMz(fLn::AbstractVector{T},v::AbstractVector{T},ℓ::Int64,nai::T2,uai::T2,vthi::T2) where{T,T2}
...
"""
# uai = 0
function fM(fLn::AbstractVector{T},v::AbstractVector{T},nai::T2,vthi::T2) where{T,T2}
...
"""
  outputs:
    fLn = fvLDMz(fLn,vGk,ℓ,nai,uai,vthi,nMod)
"""

# 1.5D, [nMod,nv],     fLn = zeros(T,nck)
function fvLDMz(fLn::AbstractVector{T},vGk::AbstractVector{T},ℓ::Int64,nai::AbstractVector{T2},
    uai::AbstractVector{T2},vthi::AbstractVector{T2},nMod::Int64) where{T,T2}

    k = 1
    fLn = fvLDMz(fLn,vGk,ℓ,nai[k],uai[k],vthi[k])
    fvLs = zero.(fLn)
    for k in 2:nMod
        if nai[k] > 0
            fvLs = fvLDMz(fvLs,vGk,ℓ,nai[k],uai[k],vthi[k])
        end
        fLn += fvLs
    end
    return fLn
...
function fvLDMz(fvL::AbstractArray{T,N},v::AbstractVector{T},LM::Int64,
    nai::T2,uai::T2,vthi::T2;L_limit::Int64=25,rel_dfLM::Real=eps0,abs_dfLM::Real=eps0) where{T,T2,N}
...
"""
  outputs:
    LM, fvL = fvLDMz(fvL,vGk,nck,LM,nai,uai,vthi,nMod;
            L_limit=L_limit,rel_dfLM=rel_dfLM,abs_dfLM=abs_dfLM)
"""

# 2.5D, [nMod,nv,LM1],     fvL = zeros(T,nck,L_limit+1)
function fvLDMz(fvL::AbstractArray{T,N},vGk::AbstractVector{T},nck::Int,LM::Int64,
    nai::AbstractVector{T2},uai::AbstractVector{T2},vthi::AbstractVector{T2},nMod::Int64;
    L_limit::Int64=25,rel_dfLM::Real=eps0,abs_dfLM::Real=eps0) where{T,T2,N}

    k = 1
    LM, fvL = fvLDMz(fvL,vGk,LM,nai[k],uai[k],vthi[k];
            L_limit=L_limit,rel_dfLM=rel_dfLM,abs_dfLM=abs_dfLM)
    for k in 2:nMod
        if nai[k] > 0
            fvLs = zeros(T,nck,L_limit+1)
            LMs = 0
            LMs, fvLs = fvLDMz(fvLs,vGk,LMs,nai[k],uai[k],vthi[k];
                    L_limit=L_limit,rel_dfLM=rel_dfLM,abs_dfLM=abs_dfLM)
            if LMs ≤ LM
                fvL[:,1:LMs+1] += fvLs
            else
...
"""
  ...
  outputs:
    LM, fvL = fvLDMz(fvL,vGk,LM,ns,nai,uai,vthi;
            L_limit=L_limit,rel_dfLM=rel_dfLM,abs_dfLM=abs_dfLM)
"""

# 3D, [nv,LM1,ns],     fvL = zeros(T,nck,L_limit+1,ns)
function fvLDMz(fvL::AbstractArray{T,N},v::AbstractVector{T},LM::Vector{Int},ns::Int64,
    nai::AbstractVector{T2},uai::AbstractVector{T2},vthi::AbstractVector{T2};
    L_limit::Int64=25,rel_dfLM::Real=eps0,abs_dfLM::Real=eps0,is_LM1_full::Bool=false) where{T,T2,N}

    for isp in 1:ns
        if nai[isp][1] > 0
            if uai[isp][1] == 0.0
                LM[isp] = 1
                fvL[:,1,isp] = fM(fvL[:,1,isp],v,nai[isp][1],vthi[isp][1])
            else
                LM[isp] = 0
                L1 = 1    # ℓ = 0
                fvL[:,L1,isp] = fvLDMz(fvL[:,L1,isp],v,0,nai[isp][1],uai[isp][1],vthi[isp][1])
                is_f = fvL[:,1,isp] .> eps0
                fs = fvL[is_f,1,isp] .+ eps0    # `for df_M = 1./fs`
                if uai[isp][1] > 0
                    for L1 in 2:L_limit+1
                        fvL[:,L1,isp] = fvLDMz(fvL[:,L1,isp],v,L1-1,nai[isp][1],uai[isp][1],vthi[isp][1])
                        LM[isp] += 1
                        fs .+= fvL[is_f,L1,isp]
                        if maximum(fvL[is_f,L1,isp]) ≤ abs_dfLM
                            # LM[isp] < 2 || (LM[isp] -= 1)
                            break
                        else
                            df_M = maximum(fvL[is_f,L1,isp] ./ fs)
                            if df_M ≤ rel_dfLM
                                # LM[isp] < 2 || (LM[isp] -= 1)
                                break
                            end
                    ...
                    if isodd(LM[isp])
                        fLnmax = maximum(abs.(fvL[:,LM[isp]+1,isp]))
                    else
                        fLnmax = maximum(fvL[:,LM[isp]+1,isp])
                    end
                    if fLnmax ≤ abs_dfLM
                        fvL[:,LM[isp]+1,isp] .= 0.0
                        if LM[isp] == 1
...
"""
  ...
  outputs:
    LM, fvL = fvLDMz(fvL,vGk,nck,LM,ns,nai,uai,vthi,nMod;L_limit=L_limit,
                    rel_dfLM=rel_dfLM,abs_dfLM=abs_dfLM,is_LM1_full=is_LM1_full)
"""

# 3.5D,  [nMod,nv,LM1,ns]
function fvLDMz(fvL::AbstractArray{T,N},vGk::AbstractVector{T},nck::Int,LM::Vector{Int},ns::Int64,
    nai::Vector{AbstractVector{T}},uai::Vector{AbstractVector{T}},vthi::Vector{AbstractVector{T}},nMod::Vector{Int};
    L_limit::Int64=25,rel_dfLM::Real=eps0,abs_dfLM::Real=eps0,is_LM1_full::Bool=false) where{T,N}

    for isp in 1:ns
        LM[isp],fvL2 = fvLDMz(fvL[:,:,isp],vGk,nck,LM[isp],nai[isp],uai[isp],vthi[isp],nMod[isp];
                L_limit=L_limit,rel_dfLM=rel_dfLM,abs_dfLM=abs_dfLM)
        fvL[:,1:LM[isp]+1,isp] = fvL2
    end
    LM1 = maximum(LM) + 1
    if is_LM1_full
        fvL = fvL[:,1:LM1,:]
        LM, fvL = fvLDMzfull(fvL,vGk,nck,LM,LM1,ns,nai,uai,vthi,nMod)
        return LM, fvL
    else
        if LM1 > 0
            for isp in 1:ns
...
"""
  ...
  outputs:
    LM, fvL = fvLDMz(fvL,vGk,nck,LM,LM1,ns,nai,uai,vthi,nMod)
"""
# is_LM1_full = true
# 3.5D,  [nMod,nv,LM1,ns]
function fvLDMzfull(fvL::AbstractArray{T,N},vGk::AbstractVector{T},nck::Int,LM::Vector{Int},LM1::Int,ns::Int64,
    nai::Vector{AbstractVector{T}},uai::Vector{AbstractVector{T}},vthi::Vector{AbstractVector{T}},nMod::Vector{Int}) where{T,N}

    for isp in 1:ns
        fvL[:,:,isp] = fvLDMzfull(fvL[:,:,isp],vGk,nck,LM1,nai[isp],uai[isp],vthi[isp],nMod[isp])
    end
    LM .= LM1 - 1
    return LM, fvL
end
...
# 2.5D, [nMod,nv,LM1],     fvL = zeros(T,nck,L_limit+1)
function fvLDMzfull(fvL::AbstractArray{T,N},vGk::AbstractVector{T},nck::Int,LM1::Int64,
    nai::AbstractVector{T2},uai::AbstractVector{T2},vthi::AbstractVector{T2},nMod::Int64) where{T,T2,N}

    k = 1
    fvL = fvLDMzfull(fvL,vGk,LM1,nai[k],uai[k],vthi[k])
    for k in 2:nMod
        if nai[k] > 0
            fvL += fvLDMzfull(zero.(fvL),vGk,LM1,nai[k],uai[k],vthi[k])
        end
    end
    return fvL
end
...
function fvLDMzfull(fvL::AbstractArray{T,N},v::AbstractVector{T},
    LM1::Int64,nai::T2,uai::T2,vthi::T2) where{T,T2,N}
